extends CanvasLayer
## Shows/hides and enables/disables the on-screen touch controls. Only
## appears on devices that actually report a touchscreen; desktop players
## never see it. Buttons are disabled (and release their action) whenever
## the game isn't actively being played, so a finger left on a button
## during the upgrade screen or pause can't leak input back into play.
